...
def _detect_node_platform(fqdn):
    """
    Identify the managed-Kubernetes platform behind a worker node FQDN.

      EKS  ip-10-0-0-1.eu-west-3.compute.internal
      GKE  europe-west1-d.c.prod-bdfs3s.internal   (<zone>.c.<project>.internal)
      GKE  gke-<cluster>-<pool>-<hash>-<suffix>    (bare node name, no domain)

    Returns {"platform", "region", "zone", "project", "evidence"}; platform is
    empty when the FQDN matches no known pattern (self-managed / on-prem node).
    """
    info = {"platform": "", "region": "", "zone": "", "project": "", "evidence": ""}
    if not fqdn:
        return info
    parts = str(fqdn).strip().lower().rstrip(".").split(".")

    if "compute" in parts and "internal" in parts:
        info["platform"] = "EKS"
        info["evidence"] = "compute.internal"
        for p in parts:
            if p and "-" in p and any(p.startswith(x) for x in
                                      ("us-", "eu-", "ap-", "ca-", "sa-", "af-", "me-", "il-")):
                info["region"] = p
                break
        return info

    # GKE node names resolve through the project's internal DNS zone.
    if len(parts) == 4 and parts[1] == "c" and parts[3] == "internal":
        import re
        info["platform"] = "GKE"
        info["project"] = parts[2]
        info["evidence"] = "c." + parts[2] + ".internal"
        # Only the zone-prefixed form carries location; the legacy gke-<cluster>-<pool>-xxxx
        # form does not, so leave region/zone empty rather than guessing.
        if re.match(r'^[a-z]+-[a-z]+\d+-[a-z]$', parts[0]):
            info["zone"] = parts[0]
            info["region"] = parts[0].rsplit("-", 1)[0]
        return info

    # XDR often reports the bare node name instead of the FQDN; GKE nodes are
    # always named gke-<cluster>-<pool>-<hash>-<suffix>.
    if len(parts) == 1 and parts[0].startswith("gke-") and parts[0].count("-") >= 3:
        info["platform"] = "GKE"
        info["evidence"] = "gke-* node name"
        return info
...
def build_markdown_card(
    ...
    if has_triage:
        ...
        node_platform = _detect_node_platform(node_fqdn)
        ...
        # ---- K8s node (EKS / GKE / self-managed) ----
        node_heading = (node_platform["platform"] + " Node") if node_platform["platform"] else "K8s Node"
        ...
        if node_platform["platform"]:
            md.append("- " + EMOJI_CHECK + " " + node_platform["platform"] +
                      " node confirmed (" + node_platform["evidence"] + ")")
        if node_platform["project"]:
            md.append("- **GCP Project**: `" + node_platform["project"] + "`")
        if node_platform["zone"]:
            md.append("- **Zone**: `" + node_platform["zone"] + "`")
        if node_platform["region"]:
            region_label = "AWS Region" if node_platform["platform"] == "EKS" else "Region"
            md.append("- **" + region_label + "**: `" + node_platform["region"] + "`")
        if node_ips:
...
def main():
    try:
        args = demisto.args()
        demisto.info("=== CodeToCloudPivot v1.3.1 START ===")
        ...
        demisto.info("=== CodeToCloudPivot v1.3.1 END ===")
